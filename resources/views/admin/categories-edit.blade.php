...
<head>
    ...
    <title>Edit Kategori - Admin MartPlace</title>
        ...
        <div class="p-6 lg:p-8">
            ...
            <div class="mb-8">
                <div class="flex items-center space-x-4 mb-2">
                    ...
                    <div class="w-12 h-12 bg-gradient-to-br from-orange-500 to-amber-500 rounded-2xl flex items-center justify-center shadow-xl">
                        <i class="fas fa-edit text-white text-xl icon-float"></i>
                    </div>
                    <div>
                        <h1 class="text-4xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
                            Edit Kategori
                        </h1>
                        <p class="text-gray-600 mt-1">Ubah informasi kategori produk</p>
                    </div>
                </div>
                <p class="text-gray-600">Edit informasi kategori: {{ $category->name }}</p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-2">
                                <input type="text" id="icon" name="icon" value="{{ old('icon', $category->icon) }}"
                                    ...
                                    placeholder="fa-tag">
                                <div class="h-10 w-10 bg-green-100 rounded-lg flex items-center justify-center" x-data="{ iconClass: '{{ old('icon', $category->icon ?? 'fa-tag') }}' }">
                    ...
                    <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                        <h3 class="text-sm font-semibold text-gray-700 mb-2">Informasi Kategori</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                                <span class="text-gray-500">ID:</span>
                                <span class="font-medium text-gray-800">{{ $category->id }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Slug:</span>
                                <span class="font-medium text-gray-800">{{ $category->slug }}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span class="font-medium text-gray-800">{{ $category->created_at->format('d M Y') }}</span>
                            </div>
                        </div>
                    </div>
