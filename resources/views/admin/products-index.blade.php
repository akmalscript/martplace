...
<head>
    ...
    <title>Kelola Produk - Admin MartPlace</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body class="bg-gray-50" x-data="{ sidebarOpen: false, showDeleteModal: false, deleteProductId: null, deleteProductName: '' }">
    <!-- Navbar -->
        ...
        <div class="p-6 lg:p-8">
            ...
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-800 mb-2">
                    <i class="fas fa-boxes mr-3 text-green-600"></i>Kelola Produk
                </h1>
                <p class="text-gray-600">Kelola dan pantau seluruh produk di platform</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-800">{{ number_format($activeProducts) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-gray-600 text-sm">Produk Disuspend</p>
                            <p class="text-2xl font-bold text-gray-800">{{ number_format($suspendedProducts) }}</p>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-semibold text-gray-900 max-w-xs truncate">{{ $product->name }}</div>
                                            <div class="text-xs text-gray-500">ID: {{ $product->id }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $product->seller->store_name ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if($product->is_active)
                                        <form action="{{ route('admin.products.suspend', $product->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-yellow-600 hover:text-yellow-900" title="Suspend Produk">
                                        ...
                                        <form action="{{ route('admin.products.activate', $product->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-green-600 hover:text-green-900" title="Aktifkan Produk">
                                                <i class="fas fa-play-circle"></i>
                                            </button>
                                        </form>
                                        @endif
                                        <button type="button" 
                                            @click="showDeleteModal = true; deleteProductId = {{ $product->id }}; deleteProductName = '{{ addslashes($product->name) }}'"
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                ...
                <p class="text-gray-600 mb-6">
                    Apakah Anda yakin ingin menghapus produk <strong x-text="deleteProductName"></strong>? Tindakan ini tidak dapat dibatalkan.
                </p>
