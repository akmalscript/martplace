                ...
                <div class="hidden md:flex flex-1 max-w-md mx-8" x-data="{
                    ...
                    <form @submit.prevent="submitSearch" class="relative w-full">
                        <input type="text" x-model="search" @input.debounce.300ms="getSuggestions"
                            @focus="showSuggestions = suggestions.length > 0" @click.away="showSuggestions = false"
                        ...
                        <div x-show="showSuggestions || loading" x-transition
                            class="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                            <template x-if="loading">
                                <div class="p-4 text-center text-gray-500">Mencari...</div>
                            </template>
                            <template x-if="!loading && suggestions.length > 0">
                                <div>
                                    <div class="px-4 py-2 text-xs text-gray-500 border-b">Saran Pencarian</div>
                                    <template x-for="(suggestion, index) in suggestions" :key="index">
                                        <button type="button" @click="selectSuggestion(suggestion)"
                                            class="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center group border-b last:border-b-0">
                                            <svg class="w-4 h-4 text-gray-400 mr-3" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                            </svg>
                                            <span class="text-gray-700 group-hover:text-green-600"
                                                x-text="suggestion"></span>
                                        </button>
                                    </template>
                                </div>
                            </template>
